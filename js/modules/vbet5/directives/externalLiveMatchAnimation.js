    ...
    return {
        ...
        link: function ($scope) {
            ...
            (function init() {
                ...
                if (Config.main.externalAnimation.livematchpro && Config.main.externalAnimation.livematchpro.enabled ) { // external animation  "livematchpro"
                    ...
                    $scope.initLiveMatchProWidget = function initLiveMatchProWidget(id) {
                        function createWidget() {
                            if (window.STATSCORE) {
                                ...
                                trackerGenerator.create('#statscore-tracker-container', id, id, Config.main.externalAnimation.livematchpro.config_id, {
                                    lang: LanguageCodes[Config.env.lang],
                                    timezone: Config.env.selectedTimeZone,
                                    configuration: Config.main.externalAnimation.livematchpro.configs
                                });
                ...
                } else if (Config.main.externalAnimation.betradar && Config.main.externalAnimation.betradar.enabled) {

                    var settings = Config.main.externalAnimation.betradar;
                    if (!window.SIR) {
            ...
            if ($scope.externalAnimation) {
                $scope.$watch('openGame.id', function (id, old) {
                    if (Config.main.externalAnimation.livematchpro && Config.main.externalAnimation.livematchpro.enabled) { // external animation  "livematchpro"
                        if (id) {
                            $scope.initLiveMatchProWidget(id, id !== old ? old : false);
                        }
                    } else if (Config.main.externalAnimation.betradar && Config.main.externalAnimation.betradar.enabled) {
                        if (id) {
                            $scope.initBetradarWidget(id);
                        }
                    }
                });
            }
