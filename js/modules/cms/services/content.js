...
CMS.service('content', ['WPConfig', 'Config', '$http', '$rootScope', function (WPConfig, Config, $http, $rootScope) {
    ...
    content.getSports = function getSports() {
        ...
        return $http.get(newsUrl + '?base_host=' + newsBaseHost + addHttpsFlag(newsUrl) + '&json=get_category_index&lang=' + newsLang + '&parent=' + (WPConfig.news.langRootCategories[lang] ||  WPConfig.news.langRootCategories.eng));
    };
    ...
    content.getRecentNews = function getRecentNews(count, categoryId, customNewsUrl) {
        ...
        return $http.get(requestUrl + '?base_host=' + (customNewsBaseHost || newsBaseHost) + addHttpsFlag(requestUrl) + '&json=get_recent_posts&lang=' + newsLang +  countStr + categoryStr  + excludedFields + tags);
    };
     ...
     * @returns {Object} promise
     */
    content.getPopups = function getPopups(cache) {
        var url = WPConfig.wpUrl + '?base_host=' + WPConfig.wpBaseHost +  addHttpsFlag(WPConfig.wpUrl) + '&lang=' + Config.env.lang + '&json=get_popup' + excludedFields + ($rootScope.geoCountryInfo && $rootScope.geoCountryInfo.countryCode ? '&country=' + $rootScope.geoCountryInfo.countryCode : '');
        return $http.get(url);
    };
